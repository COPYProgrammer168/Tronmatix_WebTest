...
export default function CartSlider() {
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 h-full w-80 z-[95] shadow-2xl transition-transform duration-300 flex flex-col ${cartOpen ? 'translate-x-0' : 'translate-x-full'}`}
        ...
        <div className="flex items-center justify-between p-4 flex-shrink-0" style={{ borderBottom: `1px solid ${headerBd}` }}>
          <div>
            ...
            {items.length > 0 && (
              <span className="text-gray-400" style={{ fontSize: 18 }}>{items.reduce((s,i) => s+i.qty, 0)} item{items.reduce((s,i) => s+i.qty,0) !== 1 ? 's' : ''}</span>
            )}
          </div>
        ...
        <div ref={itemsRef} className="flex-1 overflow-y-auto p-4 space-y-4">
          {items.length === 0 ? (
            ...
            <div key={item.id} className="flex items-start gap-3 pb-4" style={{ borderBottom: `1px solid ${itemBd}` }}>
              <img src={item.image || '/placeholder.png'} alt={item.name} className="w-16 h-16 object-cover rounded flex-shrink-0" />
              <div className="flex-1 min-w-0">
                <p className="text-white font-bold leading-tight mb-1" style={{ fontSize: 18 }}>{item.name}</p>
                {/* Show one badge per discount that applies to this item */}
                {getItemDiscounts(item).map((d, di) => (
                  <span key={di} style={{ fontSize: 10, borderRadius: 20, padding: '2px 8px', fontWeight: 800, letterSpacing: 1, display:'inline-flex', alignItems:'center', gap:3, marginBottom: 4,
                ...
                {isItemDiscounted(item) === false && discount?.categories?.length > 0 && (
                  <span style={{ fontSize: 10, color: '#666', fontStyle: 'italic', display: 'block', marginBottom: 2 }}>not eligible</span>
                )}
                <div className="flex items-center gap-2 mt-1">
                  <button onClick={() => updateQty(item.id, -1)} className="w-6 h-6 flex items-center justify-center rounded border border-[#3a3a3a] text-primary font-bold hover:border-primary transition-colors" style={{ fontSize: 16, lineHeight: 1 }}>−</button>
              ...
              <button onClick={() => removeItem(item.id)} className="text-[#777] hover:text-red-400 transition-colors flex-shrink-0 mt-1" style={{ fontSize: 20, lineHeight: 1 }}>✕</button>
            </div>
          ))}
        </div>
          ...
          <div className="flex-shrink-0 p-4 space-y-2" style={{ borderTop: `1px solid ${headerBd}`, background: footerBg }}>
            ...
            {discount && discountAmount === 0 && (
              <div className="px-3 py-1.5 rounded-lg" style={{ background: 'rgba(249,115,22,0.08)', border: '1px solid rgba(249,115,22,0.2)' }}>
                <span style={{ fontSize: 14, color: '#F97316', fontWeight: 700 }}>🏷 {discount?.code || 'Discount'} — no eligible items</span>
              </div>
            )}
            ...
            <button onClick={clearCart} className="w-full py-2 text-[#777] font-semibold hover:text-red-400 transition-colors text-center" style={{ fontSize: 18 }}>
              Clear cart
            </button>
          </div>
        )}
