...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    axios.get('/api/auth/me')
      .then(res => { const fresh = extractUser(res.data); if (fresh) applyUser(fresh) })
      .catch((err) => { if (err.response?.status === 401) clearSession() })
  ...
  const login = useCallback(async (username, password) => {
    setLoading(true)
    try {
      // FIX: trim + lowercase — mobile keyboards capitalize/add spaces
  ...
  const register = useCallback(async (username, email, password, confirm) => {
    setLoading(true)
    try {
      // FIX: trim + lowercase username AND email — consistent with login
      // Without this: register "Smos" but login "smos" → 422 mismatch
