    ...
    <div className="max-w-[1280px] mx-auto px-4 py-8" style={{ background: bg, minHeight: '60vh' }}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-black" style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 22, color: text }}>MY FAVORITES</h1>
          <span className="bg-primary text-white rounded-full px-3 py-0.5 font-bold" style={{ fontSize: 13 }}>{favorites.length}</span>
        </div>
        <Link to="/" className="text-primary font-bold hover:underline" style={{ fontSize: 16 }}>← Continue Shopping</Link>
      </div>

      {favorites.length === 0 ? (
        <div className="text-center py-24">
          <svg className="w-20 h-20 mx-auto mb-4" fill="none" stroke={dark ? '#374151' : '#e5e7eb'} strokeWidth={1.5} viewBox="0 0 24 24">
            <path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/>
          </svg>
          <h3 className="font-black mb-2" style={{ fontSize: 22, color: textSub }}>No favorites yet</h3>
          <p className="mb-6" style={{ fontSize: 15, color: textSub }}>Click the ❤️ icon on any product to save it here</p>
          <Link to="/" className="bg-primary text-white font-bold px-8 py-3 rounded-xl hover:bg-orange-600 transition-colors" style={{ fontSize: 16 }}>
            Browse Products
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {favorites.map(product => (
            <div key={product.id} className="rounded-lg overflow-hidden hover:shadow-xl transition-shadow relative flex flex-col group"
              ...
              <div className="p-3 text-center flex flex-col flex-1">
                <Link to={`/product/${product.id}`}>
                  <h3 className="font-bold mb-1 hover:text-primary transition-colors"
                    style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 14, color: text }}>{product.name}</h3>
                </Link>
                ...
                <button onClick={() => addItem(product)}
                  ...
                  style={{ fontFamily: 'Rajdhani, sans-serif', fontSize: 13, letterSpacing: 1, background: btnBg }}>
                  ADD TO CART
                </button>
              </div>
            </div>
