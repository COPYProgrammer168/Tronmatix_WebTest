...
@section('title', strtoupper(__('dashboard.nav.discounts')))
...
@section('content')
    ...
    @else
        @php
            $activeCount = $discounts
                ...
                ->filter(
                    fn($d) => !($d->expires_at && $d->expires_at->isPast()) &&
        ...
        <div class="disc-stats">
            @foreach ([['label' => 'ACTIVE', 'val' => $activeCount, 'color' => '#22c55e'], ['label' => 'EXPIRED', 'val' => $expiredCount, 'color' => '#ef4444'], ['label' => 'EXHAUSTED', 'val' => $exhaustedCount, 'color' => '#a855f7'], ['label' => 'DISABLED', 'val' => $disabledCount, 'color' => '#6b7280'], ['label' => 'TOTAL USES', 'val' => $totalUses, 'color' => '#F97316']] as $stat)
                <div
                    ...
                    <div
                        style="font-size: var(--title-size); letter-spacing:2px; color:rgba(255,255,255,0.4); margin-bottom:4px;">
                        {{ $stat['label'] }}</div>
                    <div style="font-size: var(--title-size); font-weight:900; color:{{ $stat['color'] }};">
                        {{ $stat['val'] }}</div>
                </div>
            @endforeach
        </div>
                                ...
                                $status = !$d->is_active
                                    ...
                                    : ($isExpired
                                        ...
                                        : ($isExhausted
                                            ...
                                            : 'active'));
                                $usagePct = $d->max_uses
                                    ? min(100, round(($d->used_count / $d->max_uses) * 100))
                                    : null;
                            ...
                            <tr>
                                ...
                                <td>
                                    <div style="display:flex; align-items:center; gap:8px;">
                                        <span
                                            style="font-family:monospace; font-weight:700; color:#F97316; font-size: var(--title-size); letter-spacing:1px; white-space:nowrap;">{{ $d->code }}</span>
                                ...
                                <td>
                                    @if ($dKind === 'badge')
                                        <span
                                            style="font-size: var(--title-size); font-weight:800; letter-spacing:1px; padding:3px 9px; border-radius:20px;
                                ...
                                <td>
                                    <div style="display:flex; flex-direction:column; gap:3px;">
                                        <span class="badge badge-gray"
                                            style="width:fit-content; font-size: var(--title-size);">
                                            {{ $d->type === 'percentage' ? 'PERCENT %' : 'FIXED $' }}
                                        </span>
                                        <span style="font-weight:900; color:#fff; font-size: var(--title-size);">
                                    ...
                                    @elseif($d->categories && count($d->categories) > 0)
                                        <div style="display:flex; flex-wrap:wrap; gap:4px; max-width:180px;">
                                            @foreach ($d->categories as $cat)
                                                <span class="badge badge-gray"
                                                    style="font-size: var(--title-size);">{{ $cat }}</span>
                                            @endforeach
                                        </div>
                                    @else
                                        <span
                                ...
                                <td>
                                    <div style="min-width:85px;">
                                        <div style="display:flex; justify-content:space-between; margin-bottom:4px;">
                                            <span
                                                style="color:rgba(255,255,255,0.8); font-weight:700; font-size: var(--title-size);">
                                                {{ $d->used_count }}
                                                <span style="color:rgba(255,255,255,0.3); font-weight:400;">/
                                                    {{ $d->max_uses ?? '∞' }}</span>
                                            </span>
                                            @if ($usagePct !== null)
                                                <span
                                                    style="font-size: var(--title-size); color:{{ $barColor }}; font-weight:700;">{{ $usagePct }}%</span>
                                            @endif
                                        </div>
                                        @if ($d->max_uses)
                                            <div
                                                style="height:4px; background:rgba(255,255,255,0.08); border-radius:99px; overflow:hidden;">
                                                <div
                                                    style="height:100%; width:{{ $usagePct }}%; background:{{ $barColor }}; border-radius:99px;">
                                                </div>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                <td style="white-space:nowrap;">
                                    @if ($d->expires_at)
                                        <span
                                            style="color:{{ $isExpired ? '#ef4444' : 'rgba(255,255,255,0.5)' }}; font-size: var(--title-size);">{{ $d->expires_at->format('d M Y') }}</span>
                                        @if ($isExpired)
                                            <div style="font-size: var(--title-size); color:#ef4444; font-weight:700;">
                                                EXPIRED</div>
                                        @endif
                                    @else
                                        <span style="color:rgba(255,255,255,0.25); font-size: var(--title-size);">No
                                            limit</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @php
                                        $sc = [
                                            'active' => ['#22c55e', 'rgba(34,197,94,0.12)', 'rgba(34,197,94,0.3)'],
                                    ...
                                    @endphp
                                    <span
                                        style="font-size: var(--title-size); font-weight:800; letter-spacing:1px; padding:3px 10px; border-radius:20px;
        ...
        <div class="card">
            ...
            @if ($badgeDiscounts->isEmpty())
                <div style="padding:40px; text-align:center;">
                    ...
                    <div style="color:rgba(255,255,255,0.4); font-size: var(--title-size); margin-bottom:6px;">No badge
                        discounts configured yet.</div>
                    <div style="color:rgba(255,255,255,0.25); font-size: var(--title-size);">
                        Create a coupon above, then click its <strong style="color:#a78bfa;">BADGE</strong> button to
                        configure it.
                    </div>
                </div>
            @else
                <div class="badge-grid"
                    style="display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:14px; padding:18px 20px;">
                    ...
                    border-radius:14px; padding:16px 18px; display:flex; flex-direction:column; gap:12px; position:relative;">
                            <div class="badge-active" style="position:absolute; top:12px; right:14px;">
                                @if ($badgeLive)
                                    <span
                                        style="font-size: var(--title-size); font-weight:800; color:#22c55e; background:rgba(34,197,94,0.12);
                                 border:1px solid rgba(34,197,94,0.3); border-radius:20px; padding:2px 8px; letter-spacing:1px;">●
                                        LIVE</span>
                                @else
                                    <span
                                        style="font-size: var(--title-size); font-weight:800; color:#6b7280; background:rgba(107,114,128,0.1);
                                 border:1px solid rgba(107,114,128,0.2); border-radius:20px; padding:2px 8px; letter-spacing:1px;">○
                                        INACTIVE</span>
                                @endif
                            </div>
                            ...
                            <div style="display:flex; flex-direction:column; gap:5px;">
                                <div style="display:flex; align-items:center; gap:8px;">
                                    <span
                                        style="font-size: var(--title-size); color:rgba(255,255,255,0.3); letter-spacing:1px; min-width:58px;">CODE</span>
                                    <span
                                        style="font-family:monospace; font-weight:700; color:#F97316; font-size: var(--title-size); letter-spacing:1px;">{{ $d->code }}</span>
                                </div>
                                <div style="display:flex; align-items:center; gap:8px;">
                                    <span
                                        style="font-size: var(--title-size); color:rgba(255,255,255,0.3); letter-spacing:1px; min-width:58px;">VALUE</span>
                                    <span style="font-size: var(--title-size); font-weight:800; color:#fff;">
                                        {{ $d->type === 'percentage' ? $d->value . '%' : '$' . number_format($d->value, 2) }}
                                        <span
                                            style="font-size: var(--title-size); color:rgba(255,255,255,0.4); font-weight:400;">({{ $d->type }})</span>
                                    </span>
                                </div>
                            </div>
                                ...
                                @elseif($d->categories && count($d->categories) > 0)
                                    <div style="display:flex; flex-wrap:wrap; gap:4px;">
                                        @foreach ($d->categories as $cat)
                                         ...
                                         color:#a78bfa; font-size: var(--title-size); font-weight:700; border-radius:20px; padding:2px 8px;">{{ $cat }}</span>
                                        @endforeach
                                    </div>
                                @else
                                    <span
                                        style="color:rgba(255,255,255,0.3); font-size: var(--title-size); font-style:italic;">All
                                        categories (sitewide)</span>
                                @endif
                            </div>
     ...
     background:rgba(0,0,0,0.75); align-items:center; justify-content:center; padding:16px;">
            <div
                style="background:#1a1a1a; border:1px solid rgba(249,115,22,0.2); border-radius:16px;
                padding:24px; width:100%; max-width:500px; max-height:92vh; overflow-y:auto;">

                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:22px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <div
                            style="width:34px; height:34px; border-radius:9px; background:rgba(249,115,22,0.12); border:1px solid rgba(249,115,22,0.3);
                            display:flex; align-items:center; justify-content:center; font-size: var(--title-size); flex-shrink:0;">
                            🎟️</div>
                        <div>
                            <div id="couponModalTitle"
                                style="font-size: var(--title-size); font-weight:900; color:#fff; letter-spacing:2px;">ADD
                                COUPON</div>
                            <div style="font-size: var(--title-size); color:rgba(255,255,255,0.3); margin-top:1px;">
                                Customer types this code at checkout</div>
                        </div>
                    </div>
                    <button onclick="closeCouponModal()"
                        style="background:rgba(255,255,255,0.06); border:1px solid rgba(255,255,255,0.1);
                       color:rgba(255,255,255,0.5); width:32px; height:32px; border-radius:8px;
                       cursor:pointer; font-size: var(--title-size); display:flex; align-items:center; justify-content:center; flex-shrink:0;">✕</button>
                </div>
                ...
                <form id="couponForm" method="POST" onsubmit="syncProductId()">
                    ...
                    <input type="hidden" name="product_id" id="fProductShadow">
                        ...
                        <div class="modal-full">
                            ...
                            <div style="display:flex; gap:10px;">
                                ...
                                <label id="kindCodeLabel" for="fKindCode" onclick="setKind('code')"
                                    ...
                                    <input type="radio" name="kind" id="fKindCode" value="code" checked
                                        style="display:none">
                                    <span id="kindCodeDot"
                                        ...
                                        <svg id="kindCodeCheck" width="11" height="11" viewBox="0 0 11 11"
                                            fill="none">
                                            <path d="M1.5 5.5l3 3 5-5" stroke="#fff" stroke-width="2"
                                                stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </span>
                                    <div>
                                        <div id="kindCodeText"
                                            style="font-weight:800; font-size: var(--title-size); letter-spacing:1px; color:#F97316;">
                                            🎟 DISCOUNT CODE</div>
                                        <div
                                            style="font-size: var(--title-size); color:rgba(255,255,255,0.4); margin-top:1px;">
                                            Customer types code at checkout</div>
                                    </div>
                                </label>
                                ...
                                <label id="kindBadgeLabel" for="fKindBadge" onclick="setKind('badge')"
                                    ...
                                    <input type="radio" name="kind" id="fKindBadge" value="badge"
                                        style="display:none">
                                    <span id="kindBadgeDot"
                                        style="width:20px; height:20px; border-radius:5px; flex-shrink:0; transition:all .2s;
                                         border:2px solid #6b7280; background:transparent;
                                         display:flex; align-items:center; justify-content:center;">
                                        <svg id="kindBadgeCheck" width="11" height="11" viewBox="0 0 11 11"
                        ...
                        <div class="modal-full"
                            ...
                            <span id="fActiveVisual"
                                style="width:20px; height:20px; border-radius:5px; flex-shrink:0; transition:all .2s;
                                 border:2px solid #F97316; background:#F97316;
                                 display:flex; align-items:center; justify-content:center;">
                                <svg id="fActiveCheck" width="11" height="11" viewBox="0 0 11 11"
                                    fill="none">
                                    <path d="M1.5 5.5l3 3 5-5" stroke="#fff" stroke-width="2" stroke-linecap="round"
                    ...
                    <div style="margin-top:20px; display:flex; gap:12px;">
                        <button type="button" onclick="closeCouponModal()" class="btn btn-outline"
     ...
     background:rgba(0,0,0,0.78); align-items:center; justify-content:center; padding:16px; backdrop-filter:blur(4px);">
            <div
                style="background:#141414; border:1px solid rgba(249,115,22,0.2); border-radius:20px;
                ...
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:22px;">
                    <div style="display:flex; align-items:center; gap:10px;">
                        <div
                            style="width:36px; height:36px; border-radius:10px; background:rgba(249,115,22,0.12); border:1px solid rgba(249,115,22,0.3);
                            display:flex; align-items:center; justify-content:center; font-size: var(--title-size); flex-shrink:0;">
                            🏷️</div>
                        <div>
                ...
                {{-- Live preview --}}
                <div
                    style="background:#0d0d0d; border:1px solid rgba(255,255,255,0.07); border-radius:12px;
                    ...
                    border-radius:20px; font-size: var(--title-size); font-weight:800; letter-spacing:1px; transition:all .25s;">
                            <span id="bPrevIcon">🏷️</span><span id="bPrevText">SALE</span>
                        </div>
                        <div id="bPrevValue" style="font-size: var(--title-size); font-weight:900; color:#fff;"></div>
                    </div>
                </div>
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:6px;">
                        @foreach (['🏷️', '🔥', '⚡', '💥', '🎯', '✨', '🎁', '💎', '🚀', '⭐', '🆕', '💰'] as $ico)
                            <button type="button" onclick="selectBIcon('{{ $ico }}')"
                                data-bico="{{ $ico }}"
                                style="width:36px; height:36px; border-radius:8px; font-size: var(--title-size); cursor:pointer;
                    ...
                    <div style="display:flex; flex-wrap:wrap; gap:6px;">
                        ...
                        @foreach ($bcp as $i => $p)
                            <button type="button" data-bbg="{{ $p['bg'] }}" data-bbd="{{ $p['bd'] }}"
                                ...
                                style="padding:5px 12px; border-radius:20px; cursor:pointer; font-size: var(--title-size); font-weight:800; letter-spacing:1px;
                ...
                <div style="display:flex; gap:10px;">
                    <button type="button" onclick="closeBadgeModal()"
                        style="flex:1; padding:10px; border-radius:10px; cursor:pointer; font-size: var(--title-size); font-weight:700;
                    ...
                    <button type="button" onclick="saveBadge(false)"
                        style="flex:2; padding:10px; border-radius:10px; cursor:pointer; font-size: var(--title-size); font-weight:800;
     ...
     background:#22c55e; color:#fff; font-weight:700; font-size: var(--title-size); letter-spacing:1px;
     padding:10px 20px; border-radius:10px; box-shadow:0 4px 20px rgba(0,0,0,0.4);">
            ✓ Code copied!
        </div>
        <div id="badgeCopiedToast"
            style="display:none; position:fixed; bottom:24px; right:24px; z-index:9999;
     background:linear-gradient(135deg,#a78bfa,#8b5cf6); color:#fff; font-weight:700; font-size: var(--title-size);
    ...
    `).join('')
            ...
            function openCouponModal(id, code, type, value, minOrder, maxUses, expires, isActive, categories, kind, productId,
                ...
                if (id) {
                    ...
                    document.getElementById('fType').value = type || 'percentage'
                    document.getElementById('fValue').value = value !== null && value !== undefined ? value : ''
                    document.getElementById('fMinOrder').value = minOrder !== null && minOrder !== undefined && minOrder !== 0 ?
                        minOrder : ''
            ...
            function openBadgeModal(discountId, existingBadge, discountType, discountValue) {
                ...
                _bDiscountType = discountType || null;
                _bDiscountVal = discountValue || null
            ...
            function selectBColor(btn) {
                _bBg = btn.dataset.bbg;
                _bBd = btn.dataset.bbd;
                _bC = btn.dataset.bc
                document.querySelectorAll('[data-bbg]').forEach(b => {
            ...
            async function saveBadge(clear) {
                ...
                try {
                    ...
                    toast.style.background = clear ? 'linear-gradient(135deg,#ef4444,#dc2626)' :
            ...
            function setKind(kind) {
                ...
                if (kind === 'code') {
                    cL.style.border = '2px solid rgba(249,115,22,0.5)';
                    cL.style.background = 'rgba(249,115,22,0.12)'
                    cD.style.background = '#F97316';
                    cD.style.borderColor = '#F97316'
                    cK.style.opacity = '1';
                    cT.style.color = '#F97316'
                } else {
                    cL.style.border = '2px solid rgba(255,255,255,0.1)';
                    cL.style.background = 'rgba(255,255,255,0.03)'
                    cD.style.background = 'transparent';
                    cD.style.borderColor = '#6b7280'
                    cK.style.opacity = '0';
                    cT.style.color = 'rgba(255,255,255,0.5)'
                }
                ...
                if (kind === 'badge') {
                    bL.style.border = '2px solid rgba(167,139,250,0.5)';
                    bL.style.background = 'rgba(167,139,250,0.12)'
                    bD.style.background = '#a78bfa';
                    bD.style.borderColor = '#a78bfa'
                    bK.style.opacity = '1';
                    bT.style.color = '#a78bfa'
                } else {
                    bL.style.border = '2px solid rgba(255,255,255,0.1)';
                    bL.style.background = 'rgba(255,255,255,0.03)'
                    bD.style.background = 'transparent';
                    bD.style.borderColor = '#6b7280'
                    bK.style.opacity = '0';
    ...
    @push('styles')
        <style>
            ...
            [data-theme="light"] #badgeModal [style*="color:rgba(255,255,255,0.4)"] {
                color: rgba(15, 23, 42, 0.45) !important;
            }

            [data-theme="light"] #couponModal [style*="color:rgba(255,255,255,0.3)"],
            [data-theme="light"] #badgeModal [style*="color:rgba(255,255,255,0.3)"] {
                color: rgba(15, 23, 42, 0.35) !important;
            }

            [data-theme="light"] #couponModal [style*="color:rgba(255,255,255,0.5)"],
            [data-theme="light"] #badgeModal [style*="color:rgba(255,255,255,0.5)"] {
                color: rgba(15, 23, 42, 0.55) !important;
            }

            [data-theme="light"] #couponModal [style*="background:rgba(255,255,255,0.04)"],
            [data-theme="light"] #badgeModal [style*="background:rgba(255,255,255,0.04)"] {
            ...
            [data-theme="light"] .discount-card [style*="color:rgba(255,255,255,0.3)"] {
                color: rgba(15, 23, 42, 0.35) !important;
            }

            [data-theme="light"] .discount-card [style*="background:rgba(255,255,255,0.05)"],
            [data-theme="light"] .discount-card [style*="background:rgba(255,255,255,0.06)"] {
                background: rgba(15, 23, 42, 0.04) !important;
            }

            [data-theme="light"] .discount-card [style*="border:1px solid rgba(255,255,255,0.08)"] {
                border-color: rgba(15, 23, 42, 0.08) !important;
            }

            /* Stats strip */
            [data-theme="light"] .discount-stat [style*="background:rgba(255,255,255,0.06)"] {
