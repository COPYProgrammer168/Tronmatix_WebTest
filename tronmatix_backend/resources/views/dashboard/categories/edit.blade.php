...
<div style="max-width:800px; margin:0 auto; padding:24px;">
    <div style="margin-bottom:24px;">
        <a href="{{ route('dashboard.categories.index') }}" style="
            color:var(--text-muted); text-decoration:none; font-size:14px; font-weight:600;
            transition:color .2s;
        " onmouseover="this.style.color='#F97316'" onmouseout="this.style.color='var(--text-muted)'">← Back to categories</a>
        <h1 style="font-size:var(--text-2xl); font-weight:800; letter-spacing:2px; color:var(--text); margin:12px 0 4px;">EDIT CATEGORY</h1>
        <p style="font-size:var(--text-sm); color:var(--text-muted);">Level 1 of the navigation tree</p>
    </div>
        ...
        <form method="POST" action="{{ route('dashboard.categories.update', $category) }}" enctype="multipart/form-data">
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $category->name) }}" required class="s-input" style="margin-top:6px;" placeholder="e.g. PC BUILD">
                    <div style="font-size:12px; color:var(--text-xfaint); margin-top:4px;">Current slug: /{{ $category->slug }} — regenerated automatically when the name changes.</div>
                    @error('name') <div style="color:#ef4444; font-size:13px; margin-top:4px;">{{ $message }}</div> @enderror
                </div>

                {{-- Current image --}}
                <div id="currentImageSection" style="display:{{ $category->image ? 'block' : 'none' }};">
                    <label class="block font-bold mb-2" style="font-size:13px; font-weight:700; letter-spacing:1.5px; color:var(--text-muted);">CURRENT IMAGE</label>
                    <div style="display:flex; align-items:center; gap:16px; margin-top:6px;">
                        <img src="{{ storage_url($category->image) }}" alt="" style="width:64px; height:64px; border-radius:10px; object-fit:cover; background:var(--surface-2); border:1px solid var(--border);">
                ...
                <div>
                    <label class="block font-bold mb-2" style="font-size:13px; font-weight:700; letter-spacing:1.5px; color:var(--text-muted);">IMAGE</label>
                ...
                <div>
                    ...
                    <input type="url" name="image_url" value="{{ old('image_url') }}" class="s-input" style="margin-top:6px;" placeholder="https://example.com/image.png"
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    <div>
                        ...
                        <input type="number" name="order" value="{{ old('order', $category->order) }}" min="0" class="s-input" style="margin-top:6px;">
                    ...
                    <div>
                        ...
                        <label style="display:flex; align-items:center; gap:8px; margin-top:10px; cursor:pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $category->is_active) ? 'checked' : '' }} style="width:18px; height:18px; accent-color:#F97316;">
                ...
                <div style="display:flex; gap:12px; margin-top:8px;">
                    ...
                    <a href="{{ route('dashboard.categories.index') }}" class="btn" style="
