...
<div style="max-width:800px; margin:0 auto; padding:24px;">
    <div style="margin-bottom:24px;">
        ...
        <h1 style="font-size:var(--text-2xl); font-weight:800; letter-spacing:2px; color:var(--text); margin:12px 0 4px;">ADD MAIN CATEGORY</h1>
        <p style="font-size:var(--text-sm); color:var(--text-muted);">Level 2 of the navigation tree (Category → MainCate → SubCate → Brand)</p>
    </div>

    <div style="background:var(--surface); border:1px solid var(--border); border-radius:16px; padding:24px;">
        <form method="POST" action="{{ route('dashboard.main-categories.store') }}">
            @csrf
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                <div>
                    ...
                    <div style="font-size:12px; color:var(--text-xfaint); margin-top:4px;">Only active categories are shown.</div>
