...
<div style="max-width:800px; margin:0 auto; padding:24px;">
    <div style="margin-bottom:24px;">
        ...
        <p style="font-size:var(--text-sm); color:var(--text-muted);">Level 4 of the navigation tree</p>
    </div>
        ...
        <form method="POST" action="{{ route('dashboard.brands.update', $brand) }}" enctype="multipart/form-data">
            ...
            <div style="display:flex; flex-direction:column; gap:20px;">
                <div>
                    ...
                    <select name="sub_category_id" required class="s-input" style="margin-top:6px;">
                        ...
                        @foreach($subCategories as $sc)
                            <option value="{{ $sc->id }}" {{ old('sub_category_id', $brand->sub_category_id) == $sc->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $brand->name) }}" required class="s-input" style="margin-top:6px;" placeholder="e.g. INTEL 12TH">
                    <div style="font-size:12px; color:var(--text-xfaint); margin-top:4px;">Current slug: /{{ $brand->slug }} — regenerated automatically when the name changes.</div>
                    @error('name') <div style="color:#ef4444; font-size:13px; margin-top:4px;">{{ $message }}</div> @enderror
                </div>

                @if($brand->image)
                <div id="currentImageSection" style="display:block;">
                    <label class="block font-bold mb-2" style="font-size:13px; font-weight:700; letter-spacing:1.5px; color:var(--text-muted);">CURRENT LOGO</label>
                    <div style="display:flex; align-items:center; gap:16px; margin-top:6px;">
                        <img src="{{ storage_url($brand->image) }}" alt="" style="width:64px; height:64px; border-radius:10px; object-fit:cover; background:var(--surface-2); border:1px solid var(--border);">
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
                    ...
                    <div>
                        ...
                        <label style="display:flex; align-items:center; gap:8px; margin-top:10px; cursor:pointer;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $brand->is_active) ? 'checked' : '' }} style="width:18px; height:18px; accent-color:#F97316;">
